...
#14)POCAM_num
    ...
    #Time residual graph is very sensitive to the POCAM interval
        if POCAM_num ==['P1']:
            self.estimate_peak = 400100.71     #an approximation of POCAM interval for POCAM_1
        if POCAM_num == ['P2']:
            self.estimate_peak = 400101.33     #an approximation of POCAM interval for POCAM_2
...
#3)med_bound decides width of the gaussian formed when time residual histogram is plotted.  
    def res(self, gaus_peak, greater_than, med_bound):
        ...
        abs_elim = self.abs_elim_3[self.rising_a_elim_3 > 0] + self.rising_a_elim_3[self.rising_a_elim_3 > 0]
        abs_time = self.abs_elim_3[self.rising_a_elim_3 > 0]
        
        for p in range(0, len(JumpIndex)):    

            # Testing
            # if p != JumpIndex.size - 1 and (JumpIndex[p+1] - JumpIndex[p]) <= 1: 
            #     continue
            # elif p == JumpIndex.size - 1 and (est_res_diff.size - JumpIndex[p]) <= 1:
            #     continue
            #if p == 1:
            #    break
            #end Testing
            ...
            #run_time selects individual runs from absolute_elim, ie considers all the events that crossed the given threshold
                ...
                run_abs_time = abs_time[(abs_elim >= 0) & (abs_elim < b)]
                ...
                run_abs_time = abs_time[(abs_elim >= a) & (abs_elim <= abs_elim[abs_elim.size-1])]
                ...
                run_abs_time = abs_time[(abs_elim >= a) & (abs_elim < b)]
        
            # Calculate gaus_peak for individual run
            #gaus_peak = self.minimizer(run_abs_time[0], run_abs_time[-1])

            time_res = v%gaus_peak #calculates the residual of events in peak for individual runs (POCAM EVENTS ONLY)
            time_res_all = run_time%gaus_peak #calculates the residual of all the events for individual runs
...
#3)sDOM_num
    ...
    #plotting 2D histogram of time residuals vs timestamps
        ...
        self.abs_elim = self.abs_elim_3[self.rising_a_elim_3 > 0] + self.rising_a_elim_3[self.rising_a_elim_3 > 0]
        x = self.abs_elim
...
#4)after the mean of  y_axis is calculated the uboud is added to it 
    def calc_res(self, BinsHist, med_bound, yaxis_lbound, yaxis_ubound):
        ...
        for d in range(0, self.JumpIndex.size + 1):
            ...
            if d == self.JumpIndex.size:
                ...
                Max = BinsHist
                #print('b', b, 'Max', Max)
                select = self.abs_elim[(self.abs_elim >= a) & (self.abs_elim <= self.xedges[b])] % self.gaus_peak #selecting timestamp values 
            ...
            #Gaussian Fit 
                ...
                time_res_sub = select - popt[0] #subtracting time_residual with the mean obtained by using curve fit all events crossing the given rising edge considered
    ...
    #plotting time residual histogram x limit (-100, 200)
        ...
        n, bins, patches = plt.hist(t_res_all, 
                               #500,
                               np.linspace(-100, 201, 300), # num_bins should be a variable. alternatively write np.arange(-100,200,1) to specify bin width
                               log = True)
        plt.axvline(x = 0, color = 'r')
        plt.axvline(x = -10, color = 'k')
        plt.axvline(x = 10, color = 'k')
